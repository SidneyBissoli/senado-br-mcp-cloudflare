...
- **SDK:** `@modelcontextprotocol/server` 2.x (instâncias de McpServer por requisição; o `@modelcontextprotocol/sdk` v1 fica só como peer do `agents`, em tempo de dev)
...
- **`consultas_votos`** — acervo **histórico** separado de votos por UF, lido do CSV Arquimedes de ~33 MB (`Proposições-com-votos.csv`), agregado a uma linha por matéria com quebra `votosPorUf`. O carimbo "dados atualizados até" do CSV vira o `data_vintage` da proveniência; fica **fora** do hash da linha (`consultaVotoCore`) para que a reingestão semanal com votos inalterados não suje o `_history` (o Senado republica o CSV periodicamente — confirmado em 20/07/2026 —, então o job semanal reingere o vintage corrente sob os mesmos guards de anomalia dos demais corpora). `STATUS ATUAL` é uniformemente "Descontinuado" — por isso é acervo, não migração das consultas abertas. Servido por `senado_ecidadania_consultas_votos` com proveniência apontando para o CSV (`ECIDADANIA_ARQUIMEDES`).
...
**Toda ferramenta** anexa um **envelope de proveniência** para que qualquer resultado seja rastreável até sua fonte oficial — a proveniência é parte essencial da resposta, não um extra opcional (o público são jornalistas e pesquisadores em ciência política, para quem um número sem fonte é inutilizável). Desde a **v3.5.0** o envelope implementa o **contrato de proveniência v1.0** do portfólio ([`@sbissoli/mcp-provenance`](https://www.npmjs.com/package/@sbissoli/mcp-provenance)): o servidor monta e valida um modelo canônico completo por resposta e emite a projeção **`concise`** — bloco de 6 chaves em ordem fixa, com `null` explícito quando a fonte não expõe o valor. O bloco fica em `structuredContent.provenance` e é espelhado como um rodapé compacto de fonte no texto (linha de fonte, linha de licença e o aviso de que a referência completa pode ser solicitada na própria conversa) — o JSON dos dados **não** é duplicado com o envelope, mantendo o custo de tokens por resposta baixo.

Campos do bloco `concise` (por resposta — uma ferramenta, uma fonte): `source`, `source_url` (URL canônica consultada), `data_vintage` (competência/vintage do dado; chamava-se `reference_period` antes da v3.5.0), `retrieved_at` (ISO-8601 da extração no upstream — preservado mesmo em cache-hit, o diferenciador nível-1), `citation` (citação pronta) e `license`. O modelo canônico por trás do bloco também carrega `dataset.id`, `api_version` e `field_sources` por campo — validados a cada build e refletidos na lista `attribution` (todas as `source_url` distintas da resposta), que acompanha o bloco no `structuredContent`.
...
| `senado_horas_extras` | Pagamentos de horas extras por mês, com totais; `estatisticas=true` devolve a distribuição do conjunto (mín/máx/média/mediana/percentis) + ranking top/bottom, ou um ranking por servidor via `agruparPor` (nome/competência), com `topN` |
