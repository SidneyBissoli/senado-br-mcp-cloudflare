 ...
 * links de entrada para o domínio são finos, então esperar rastreamento
 * espontâneo era esperar por algo que podia não acontecer — e, pior, ler o
 ...
 * desenho — ela prova posse do domínio justamente por estar servida nele.
 *
 * Aqui o sitemap tem DUAS URLs: a raiz e a página de termos. Privacidade fica
 * de fora por decisão registrada em `tests/landing.test.ts` — páginas de
 * obrigação não disputam a busca com a landing —, mas os termos descrevem o
 * serviço e são o que um avaliador de diretório procura.
 */

import { SITE_URL } from "./landing.js";

/**
 * Chave do IndexNow. É PÚBLICA por desenho: prova posse do domínio por estar
 * servida em `/<chave>.txt`, então versioná-la não é vazamento.
 */
export const INDEXNOW_KEY = "a5286cd351e45bcfa411debef2957eaf";
 ...
 * fora do índice — `/mcp` é JSON-RPC e as demais são sondas operacionais.
 */
...
export function sitemapXml(): string {
  const urls = [
    { loc: `${SITE_URL}/`, prioridade: "1.0", freq: "weekly" },
    { loc: `${SITE_URL}/terms`, prioridade: "0.3", freq: "yearly" },
  ];
  const corpo = urls
    .map(
      (u) => `  <url>
 ...
 * Chamada ANTES da auth: um rastreador não tem credencial.
 */
export function discoveryResponseForPath(pathname: string): Response | null {
  ...
  if (pathname === `/${INDEXNOW_KEY}.txt`) return texto(INDEXNOW_KEY);
  return null;
}
