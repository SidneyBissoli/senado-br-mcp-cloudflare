...
export const SERVER_INSTRUCTIONS = [
  ...
  "NUNCA transcreva vocabulário interno na resposta: nomes de parâmetros (`subordinadasA`, `estatisticas`, `campo`, `tipo`, `agruparPor`), nomes de campos/colunas do resultado (ex.: `valorTotalTransacoes`, `naoClassificados`, `afastadosOuEmTransito`, `regimeEspecial`), valores de enum crus (`tipo=atos-concessao`) nem mensagens técnicas de aviso. Frases como 'o parâmetro subordinadasA faz essa contagem' são ERRADAS — diga 'contei os servidores de toda a estrutura subordinada à unidade'. Traduza tudo para linguagem que um cidadão sem conhecimento da API entenda — use os rótulos legíveis já fornecidos no resultado, como `campoAnalisado`, `agrupadoPorRotulo`, os `rotulo` e as `nota`. O usuário não conhece — e não precisa conhecer — a arquitetura interna do serviço.",
