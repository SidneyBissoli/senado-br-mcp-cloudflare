...
export function registerSupridosTools(server: SenadoToolHost, admBaseUrl: string) {
  ...
  server.tool(
    ...
    {
      // 2013, e não o 2010 que estava aqui: medido em 24/09/2026, `/supridos/`
      // de 2009 a 2012 responde 404 e 2013 responde 200. Com o
      // `treat404AsEmpty` antigo, pedir 2011 devolvia `count: 0` — lia-se
      // "o Senado não adiantou nada em 2011". O limite fecha a porta antes da
      // rede (classe `contrato`); fora dela, a ausência agora é tipada.
      ano: z.number().int().min(2013).max(2100).describe("Ano de referência (a fonte publica de 2013 em diante)"),
