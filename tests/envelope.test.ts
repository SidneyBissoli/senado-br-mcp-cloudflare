...
import { recordProtocolMethods, type RequestTag } from "../src/instrument.js";
import {
  LeitorDeEnvelope,
  MAX_EVENTO_BYTES,
  desfechoDaResposta,
...
const SSE = {
  recusaDeEsquema:
    'event: message\ndata: {"result":{"content":[{"type":"text","text":"Input validation error: Invalid arguments for tool senado_buscar_materias: palavraChave: Invalid input: expected string, received number"}],"isError":true},"jsonrpc":"2.0","id":1}\n\n',
  toolInexistente:
  ...
  erroMole:
    'event: message\ndata: {"result":{"content":[{"type":"text","text":"{\\"resultados\\": [], \\"aviso\\": \\"Nenhuma materia casa com o termo.\\"}"}]},"jsonrpc":"2.0","id":1}\n\n',
...
describe("a linha que iria para o Analytics Engine", () => {
  ...
  it("recusa de esquema, HTTP 200: error + classe contrato (antes NÃO HAVIA LINHA)", async () => {
    ...
    const linha = await umaLinha(chamada("senado_buscar_materias", { query: 12345 }), SSE.recusaDeEsquema);
    expect(linha.blobs?.[0]).toBe("senado_buscar_materias");
  ...
  it("ausência respondida como dado segue ok e SEM classe", async () => {
    const linha = await umaLinha(chamada("senado_buscar_materias", { query: "zzzqqq" }), SSE.erroMole);
  ...
  it("chamada que o hook JÁ GRAVOU não gera linha aqui: a reconciliação fecha", async () => {
    ...
    const pontos = await linhas(
      chamada("senado_listar_senadores", {}, 7),
      SSE.sucesso,
      200,
      new Map([["senado_listar_senadores", 1]]),
    );
    expect(pontos).toEqual([]);
  });

  it("chamada válida sem recibo segue ok, e o resto da linha não muda", async () => {
    const linha = await umaLinha(chamada("senado_listar_comissoes", {}, 7), SSE.sucesso);
    expect(linha.blobs?.[1]).toBe("ok");
  ...
  it("lote: cada mensagem recebe o desfecho DELA, não o do vizinho", async () => {
    const pontos = await linhas(
      [chamada("senado_listar_senadores", {}, 1), chamada("nao_existe", {}, 2)],
